...
# This test file lives at  <gitroot>/disastermind/tests/test_deploy_hardening.py
#   * PROJECT = the nested project dir (Dockerfile, Makefile, SECURITY.md live here)
#   * GIT_ROOT = the repository root (GitHub reads .github/workflows from HERE)
...
def test_root_ci_cds_into_project_and_builds_docker() -> None:
    text = _read(ROOT_CI)
    # GitHub runs from the repo root, so the workflow must operate inside the
    # nested project dir (either a working-directory default or an explicit path).
    assert "disastermind" in text, "git-root CI must reference the nested project dir"
    assert "working-directory: disastermind" in text or "cd disastermind" in text
    # There is a job that builds the container image.

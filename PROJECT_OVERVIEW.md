...
> **Which model produced these numbers.** The figures below are generated by the
> **deterministic, stdlib-only baseline** (a fixed-seed logistic fit with
> calibration) — the same model a clean checkout runs with zero optional
> dependencies, which is why `make reproduce` regenerates each value to Δ = 0.
> The optional **XGBoost** backend is a *lazily-imported* upgrade path that falls
> back to this baseline when absent; it is **not** the source of the skill quoted
> here. The headline is "a calibrated, leak-free baseline beats the operational
> incumbents," not "a deep net does."
>
> **Does the optional backend actually help?** `make compare-backends` fits
> XGBoost on the *identical* splits + calibration and scores it against the
> baseline (CI-gated to a tolerance in `docs/backend_comparison_golden.json`).
> Measured result: a real lift on earthquakes (AUC 0.937 → **0.950**, lower
> Brier), a wash on flood (Δ AUC −0.000), and *no gain* on fire (logistic wins on
> all three metrics). So XGBoost is offered as a hazard-specific, opt-in upgrade
> while the dependency-free baseline remains the shipped default — the "XGBoost"
> name is now backed by a reproducible measurement, not asserted. *(There is no
> deep-CNN flood model; an earlier draft named a "U-Net" that was never built and
